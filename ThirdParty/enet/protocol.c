...
enet_protocol_handle_incoming_commands (ENetHost * host, ENetEvent * event, enet_uint8 useGTProtocol)
{
    ...
    size_t minLength = (useGTProtocol == 1) ? ENET_OFFSETOF(ENetGTProtocolHeader, sentTime) : ENET_OFFSETOF(ENetProtocolHeader, sentTime);

    if (host -> receivedDataLength < minLength)
        return 0;
    ...
    if (useGTProtocol == 1) {
        ...
        if (r0 > host->address.port || (r1 ^ host->address.port) != r0 || r2 < 0x920D)
            return 2;

        header = (ENetProtocolHeader *) ((enet_uint8 *) host -> receivedData + ENET_OFFSETOF(ENetGTProtocolHeader, peerID));
    } else {
    ...
    if (useGTProtocol == 1)
        headerSize += ENET_OFFSETOF(ENetGTProtocolHeader, peerID);
...
// for now forcing gt protocol for outgoing commands
// no problem with income it handles both
static int
enet_protocol_send_outgoing_commands (ENetHost * host, ENetEvent * event, int checkForTimeouts)
{
    const size_t headerSize = (host->useNewPacket == 1) ? sizeof(ENetGTProtocolHeader) : sizeof(ENetProtocolHeader);

    enet_uint8 headerData [sizeof(ENetGTProtocolHeader) + sizeof (enet_uint32)];
    ENetProtocolHeader * stdHeader = (ENetProtocolHeader *) headerData;
    ENetGTProtocolHeader * gtHeader = (ENetGTProtocolHeader *) headerData;

    int sentLength = 0;
    size_t shouldCompress = 0;
    ENetList sentUnreliableCommands;

    if (host->useNewPacket == 1)
    {
        enet_uint16 peerPort = host->peers->address.port;

        enet_uint16 randOffset = (enet_uint16)(rand() % (peerPort + 1));
        enet_uint16 randSeed   = (enet_uint16)rand();

        gtHeader->r0 = htons(randOffset);
        gtHeader->r1 = htons(peerPort ^ randOffset);
        gtHeader->r2 = htons((randSeed & 0xF3DF) | 0x920D);
    }
         ...
         ++ currentPeer)
    {
        ...
        {
            if (host->useNewPacket == 1)
                gtHeader->sentTime = ENET_HOST_TO_NET_16 (host -> serviceTime & 0xFFFF);
            else
                stdHeader->sentTime = ENET_HOST_TO_NET_16 (host -> serviceTime & 0xFFFF);
        ...
        {
            if (host->useNewPacket == 1)
                host -> buffers -> dataLength = (size_t) & ((ENetGTProtocolHeader *) 0) -> sentTime;
            else
        ...
        if (host->useNewPacket == 1)
            gtHeader->peerID = ENET_HOST_TO_NET_16 (currentPeer -> outgoingPeerID | host -> headerFlags);
        else
            stdHeader->peerID = ENET_HOST_TO_NET_16 (currentPeer -> outgoingPeerID | host -> headerFlags);
